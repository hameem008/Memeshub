...
import AlertOwn from "../components/AlertOwn";
...
function ImageUploader() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        fetch(`http://localhost:3000/uploadmeme/${caption}/${data.imageUrl}/${userId}`)
      ...
      .then((data) => {
        ...
        if(data.isSuccess){
          ...
          setAlert(true);
          setAlertType("error");
          setAlertText("Image Upload Failed");
        }
        console.log(data);
      })
      .catch((error) => {
  ...
  return (
    <Container maxW="xl">
      {alert?<AlertOwn type={alertType} text={alertText}/>:<></>}
      <Center mt={10}>
        <Stack spacing={4}>
